...
if site_choice == '다나와':
            ...
            if st.button("🔍 검색 옵션 로드", key="danawa_load_options"):
                if search_keyword.strip():
                    with st.spinner('검색 옵션을 가져오는 중...'):
                        ...
                        if not st.session_state.danawa_search_options:
    ...
    if st.session_state.danawa_search_results:
        with st.container():
            ...
            with col1:
                st.markdown(f'''<div class="metric-container"><h2>{len(st.session_state.danawa_search_results)}</h2><p>총 제품 수</p></div>''', unsafe_allow_html=True)
            with col2:
            ...
            with col3:
                def create_excel_file_danawa():
                    data = [{'No': i+1, '제품명': p.name, '가격': p.price, '세부사양': p.specifications} for i, p in enumerate(st.session_state.danawa_search_results)]
            ...
            st.markdown("<hr>", unsafe_allow_html=True)
            st.markdown("### 📋 검색 결과")
            df_display = pd.DataFrame([{'No': i+1, '제품명': p.name, '가격': p.price, '세부사양': p.specifications} for i, p in enumerate(st.session_state.danawa_search_results)])
